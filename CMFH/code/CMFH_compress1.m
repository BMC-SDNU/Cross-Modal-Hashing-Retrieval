...
code_trainX =I_db * W1;
%code_trainX= sign((bsxfun(@minus, code_trainX , mean(code_trainX,1))));% 2173¡Ábits
code_trainX=code_trainX>0;

code_trainY =T_db * W2;
%code_trainY= sign((bsxfun(@minus, code_trainY , mean(code_trainY,1))));% 2173¡Ábits
code_trainY=code_trainY>0;
% traintime=toc;
 
code_test_X= sign(I_te * W1);%((bsxfun(@minus,I_te * W1 , mean(Y,1))));%693¡Ábits
