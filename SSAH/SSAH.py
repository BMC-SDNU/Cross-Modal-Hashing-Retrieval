...
class SSAH(object):
    def __init__(self, sess, config):
        ...
        self.train_L = L['train']
        self.train_X = X['train']
        self.train_Y = Y['train']

        self.query_L = L['query']
        self.query_X = X['query']
        self.query_Y = Y['query']

        self.retrieval_L = L['retrieval']
        self.retrieval_X = X['retrieval']
        self.retrieval_Y = Y['retrieval']

        self.lr_lab = lr_lab
        self.lr_img = lr_img
        self.lr_txt = lr_txt
        self.lr_dis = lr_dis
        self.Sim = (np.dot(self.train_L, self.train_L.transpose()) > 0).astype(int)*0.999
        ...
        self.numClass = self.train_L.shape[1]
        self.dimImage = self.train_X.shape[1]
        self.dimText = self.train_Y.shape[1]
        self.dimLab = self.train_L.shape[1]
        self.num_train = self.train_L.shape[0]

        self.checkpoint_dir = checkpoint_dir
        self.savecode_dir = savecode_dir
        self.result_dir = result_dir
        self.dataname = config.dataname

        self.bit = config.bits
        ...
        self.FLAG_savecode = False
        self.phase = config.phase # train: dont load checkpoint
    ...
    def train(self):
        ...
        var['B'] = np.sign(var['H'] + var['G'] + var['F'])
        t2 = time.time()
        print('[Train time]-[%d bit]: %.2f' % (self.bit, (t2 - t1)))
    ...
    def test(self, phase):
        ...
        print('==================================================')
        print('...test map: map(i->t): %3.4f, map(t->i): %3.4f' % (test['mapi2t'], test['mapt2i']))
        # print('...test map: map(t->t): %3.4f, map(i->i): %3.4f' % (test['mapt2t'], test['mapi2i']))
        print('==================================================')
        t2 = time.time()
        print('[Test time]-[%d bit] %.2f' % (self.bit, (t2 - t1)))

        # Save hash code
        if self.FLAG_savecode:
            if not os.path.exists(savecode_dir):
                os.makedirs(savecode_dir)
            save_path = os.path.join(self.savecode_dir, self.dataname + '_' + str(self.bit) + 'bits.mat')
            sio.savemat(save_path, {'query_image': test['qBX'], 'query_text': test['qBY'],
                                    'retrieval_image': test['rBX'], 'retrieval_text': test['rBY'],
    ...
    def save(self, checkpoint_dir, step):
        model_name = "SSAH"
        model_dir = "%s_%s" % (self.dataname, self.bit)
